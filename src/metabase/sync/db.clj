(ns metabase.sync.db
  "Application database queries for the sync module. Reads here ask for the raw rows -- sync reconciles what it wrote
  against what the warehouse reports -- so Tables go through [[raw-table-query]].
...
(defn- raw-table-query
  "What a sync query reads Tables from: `table-query` with neither overlay applied. Sync reconciles what it wrote
  against what the warehouse reports, so it has to see a Table as it recorded it -- not wearing the values a user
  set, and not at the workspace table a remapping points it to."
  []
