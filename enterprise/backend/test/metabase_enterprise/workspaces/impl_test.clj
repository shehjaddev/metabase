...
(deftest remap-table!-test
  (mt/with-premium-features #{:workspaces}
    (testing "records a remapping into the database's workspace schema"
      (mt/with-temp-vals-in-db :model/Database (mt/id) {:settings {:database-workspace-schema "PUBLIC"}}
        (try
    ...
    (testing "throws when the database has no database-workspace-schema set"
      (mt/with-temp-vals-in-db :model/Database (mt/id) {:settings {:database-workspace-schema nil}}
        (is (thrown-with-msg?
